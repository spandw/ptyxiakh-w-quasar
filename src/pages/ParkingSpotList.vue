<template>
  <q-page>
    ...
    <div class="fit row wrap justify-evenly content-start q-pa-md" style="margin-top: 100px;">
      <!-- <div class="q-col-gutter-md col"> -->
      <!-- :imageUrl="item.imageUrl" :imageAlt="item.imageAlt" -->
      <ParkingSpotCard v-for="(parking, index) in filteredSpots" :key="index" :title="parking.title"
        :description="parking.description" :address="parking.address" :city="parking.city" :price="parking.price"
        :id="parking.id" :userId="parking.user_id" :deleteSpot="deleteSpot" />

      <!-- </div> -->
    </div>
    <q-page-sticky expand position="top" style="background-color: #4f759b; opacity: 100%;">

      <div class=" flex fit row wrap justify-center items-start content-center q-pa-md">
        <q-select bg-color="white" outlined v-model="selectedCity" use-input hide-selected fill-input input-debounce="0"
          :options="filteredCities" @filter="filterFn" clearable>
        ...
        <q-select bg-color="white" outlined v-model="selectedCarType" use-input hide-selected fill-input
          input-debounce="0" :options="carTypes" clearable>
        </q-select>
          ...
          <template v-slot:append>
            <q-icon name="event" class="cursor-pointer">
              <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                <q-date v-model="date" range minimal @update:model-value="validateDate" mask="DD/MM/YYYY">
...
const validateDate = () => {
  console.log("date.value.from", date.value.from, '-', "date.value.to", date.value.to, "date.value", date.value)
  if (date.value.from) {
...
const deleteSpot = (id) => {
  $q.dialog({
    ...
    console.log(id)
    parkingSpotStore.deleteSpot(id).then(parkingSpotStore.getAllParkingSpots())
  }).onOk(() => {
